...
func (c *clusterApi) getPair(w http.ResponseWriter, r *http.Request) {
	getPairRequest := &api.GetClusterPairRequest{}
	method := "getPair"

	if err := json.NewDecoder(r.Body).Decode(getPairRequest); err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	resp, err := inst.GetPair(getPairRequest)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (c *clusterApi) generatePairToken(w http.ResponseWriter, r *http.Request) {
	method := "generatePairToken"
	generatePairRequest := &api.GenerateClusterPairTokenRequest{}

	if err := json.NewDecoder(r.Body).Decode(generatePairRequest); err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	resp, err := inst.GeneratePairToken(generatePairRequest)
	if err != nil {
		c.sendError(c.name, method, w, err.Error(), http.StatusInternalServerError)
		return
	}
