...
// swagger:operation POST /osd-snapshots/groupsnap volumegroup snapshotVolumeGroup
//
...
func (vd *volAPI) snapGroup(w http.ResponseWriter, r *http.Request) {
	...
	snapRes, _ = d.GroupSnapshot(snapReq.Id, snapReq.Labels)
	json.NewEncoder(w).Encode(&snapRes)
}
...
func (vd *volAPI) Routes() []*Route {
	return []*Route{
		...
		{verb: "POST", path: snapPath("/groupsnap", volume.APIVersion), fn: vd.snapGroup},
